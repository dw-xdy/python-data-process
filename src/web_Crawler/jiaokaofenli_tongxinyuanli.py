...
def fetch_question(card_order):
    ...
    try:
        ...
        if json_data.get("code") == 200 and json_data.get("data"):
            ...
            print(
                f"序号 {card_order}: 无数据或接口返回错误 - {json_data.get('msg', '未知错误')}"
            )
            return None
...
def crawl_questions(start=1, end=175, delay=0.5):
    ...
    print("-" * 50)
    print(
        f"爬取完成！成功: {len(all_questions)} 题，失败: {end - start + 1 - len(all_questions)} 题"
    )
...
def resume_crawl(existing_file, start, end):
    ...
    print(
        f"缺失序号: {missing_numbers[:20]}..."
        if len(missing_numbers) > 20
        else f"缺失序号: {missing_numbers}"
    )
