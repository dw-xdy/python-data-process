...
def convert_to_utf8(file_path: Path) -> tuple:
    ...
    try:
        ...
        if original_encoding and original_encoding.lower() in ['utf-8', 'ascii'] and confidence > 0.9:
        ...
        if original_encoding:
            try:
                ...
                print(f"✅ 转换: {file_path.name} ({original_encoding} -> utf-8, 置信度: {confidence:.2%})")
        ...
        for encoding in common_encodings:
            try:
                ...
                file_path.write_text(content, encoding='utf-8')
                print(f"✅ 转换: {file_path.name} ({encoding} -> utf-8, 自动检测失败，使用备选编码)")
...
def convert_folder_to_utf8(folder_path: Path) -> tuple:
    ...
    for txt_file in folder_path.rglob("*.txt"):
        ...
        if success:
            ...
            encoding_stats[original_encoding] = encoding_stats.get(original_encoding, 0) + 1
